...
import com.uniwin.webkey.cms.itf.MLogService;
...
public class NewsSelDetailWindow extends Window implements AfterCompose
{
    ...
    // 管理日志数据访问接口
    MLogService               mlogService = (MLogService) SpringUtil.getBean("mlogService");
    ...
    public void initInfo(WkTDistribute dis)
    {
        ...
        if (info.getKiType().trim().equals("1"))
        {
            ...
            kitype.setValue(org.zkoss.util.resource.Labels.getLabel("newspub.ui.input"));
        ...
        } else if (info.getKiType().trim().equals("2"))
        {
            ...
            kitype.setValue(org.zkoss.util.resource.Labels.getLabel("newspub.ui.upfile"));
        } else if (info.getKiType().trim().equals("3"))
        {
            ...
            bdnews.setVisible(false);
            tupian.setVisible(false);
            kiaddress.setContent("<a href=\"" + info.getKiAddress()
                    + "\" target=\"_blank\">" + info.getKiAddress() + "</a>");
            kitype.setValue(org.zkoss.util.resource.Labels.getLabel("newspub.ui.outlink"));
        }
    ...
    {
        String path = Executions.getCurrent().getDesktop().getWebApp()
                .getRealPath("/upload/info").trim();
        String fileName = (newsService.getWkTInfo(dis.getKiId()))
                .getKiAddress().trim();
