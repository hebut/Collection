...
/**
 * <li>功能描述：模板列表页面 对应页面admin/system/parameters/TemplateManage/index.zul
 * @author bobo
...
public class IndexWindow extends Window implements AfterCompose
{
    private static final long serialVersionUID = 8307495269548211816L;
    
    Center                    cssListCen;
    
    // Listbox列表框组件
    private Listbox           cssListbox, templeListbox;
    ...
    String                    pubPath          = WebkeyControlUtil.getPath("/");

    // 登录用户的Session
    final Users               Users            = (Users) Sessions.getCurrent()
                                                   .getAttribute("users");
    ...
    public void afterCompose()
    {
        ...
        Sessions.getCurrent().setAttribute("root", map);
        try
        {
            initWindow();
        } catch (IOException e)
        ...
        Sessions.getCurrent().setAttribute("pubPath", pubPath); // 文档列表DoclistAction.java用到路径
                ...
                } else
                {
                            ...
                            | Messagebox.NO, Messagebox.QUESTION) == Messagebox.YES)
                    {
                        while (it.hasNext())
                        {
                            ...
                            File zipFile = new File(pa + "\\" + "admin" + "\\" + "styles"); // 同时删除对应的.zip
                ...
                } else
                {
                            ...
                            | Messagebox.NO, Messagebox.QUESTION) == Messagebox.YES)
                    {
                        while (it.hasNext())
                        {
                            ...
                            // 实现文件及下述文件夹的删除.查找对应路径下，是否存在同名fn的文件夹，有则删除
                            delFile("admin" + "\\" + "styles", folder, filename + ".wkt"); 
        ...
        if (folderList != null && folderList.length > 0)
        {
            for (int i = 0; i < folderList.length; i++)
            {
                if (folderList[i].isDirectory())
                {
                    ...
                    {
                        String cssPath1 = WebkeyControlUtil.getPath(path)+ "\\" + fileName;
                        ...
                        if (ftlFileList != null && ftlFileList.length > 0)
                        {
                            for (int j = 0; j < ftlFileList.length; j++)
                            {
                                        ...
                                        && ftlFileList[j].isFile())
                                {
                                    ...
                                    if (ftl.trim().equals(filename.trim()))
                                    {
    ...
    public void initWindow() throws IOException
    {
        ...
        cssListbox.setItemRenderer(new ListitemRenderer()
        {
            public void render(Listitem item, Object data) throws Exception
            {
                ...
                // 获取.css文件路径，得到其中的css文件名称
                String path = path0 + "\\" + file.getName() + "\\" + "index.css"; 
                Listcell c2;
                if (path != null)
                {
                ...
                Listcell c3 = new Listcell(albumTime.format(file.lastModified()));
    ...
    public class EditListener implements EventListener
    {
        public void onEvent(Event event) throws Exception
        {
            ...
            final NewCssWindow win = (NewCssWindow) Executions
                    .createComponents("/apps/cms/template/newCSS.zul", null, null);
            win.doModal();
        ...
        public void onEvent(Event event) throws Exception
        {
            ...
            if (fList.size() != 0)
            {
                ...
                for (int i = 0; i < fList.size(); i++)
                {
                    ...
                    {
                    	HtmlParser hp = new HtmlParser(wktFile.getPath(),baseDir0 +"\\"+ cssFilename + "\\" + wktFilename +  ".ftl", path0 +"\\"+ "head.txt", wktFilename,cssFilename);
...
//        }
    ...
    /**
     * <li>功能描述：监听TempListWindow中的Listbox事件，转移到模板编辑窗口
     * 
     * @author bobo
     * @serialData 2010-7-23
     */
    class pointListener implements EventListener
    {
        public void onEvent(Event event) throws Exception
        {
            ...
            Listitem c = (Listitem) event.getTarget().getParent();
            File file = (File) c.getValue();
            Executions.getCurrent().setAttribute("TempleListfileName",file.getName().subSequence(0,
                            file.getName().lastIndexOf('.'))); // 得到.wkt文件文件名（不包含扩展名）

            String path = WebkeyControlUtil.getPath("/admin/styles") + cf
                    + "\\" + file.getName(); // 获取.css文件路径，得到其中的css文件名称（目前，是写死的路径，以后需要该换掉）
            String resu = FileUtil.getWktDesName(path);
            Executions.getCurrent().setAttribute("tempDesc", resu); // 模板描述信息
     ...
     */
    public void onClick$newCss() throws SuspendNotAllowedException, InterruptedException
    {
    ...
    {				
		...
		if(!tabbox.getTabs().hasFellow(String.valueOf("indexnewTab"))){
			final Tab newTab=new Tab();				
			newTab.setId(String.valueOf("indexnewTab"));
			newTab.setLabel("添加模板");				
			Tabpanel newtabpanel=new Tabpanel();
			newtabpanel.setId(String.valueOf("indexnewtabpanel"));
			tabbox.getTabs().getChildren().add(newTab);
			tabbox.getTabpanels().getChildren().add(newtabpanel);
					
			Hbox h=new Hbox();	
			h.setId(String.valueOf("indexh"));					
			h.setAlign("center");
			h.setPack("center");
			h.setWidth("100%");
    ...
    public void onClick$importZip() throws InterruptedException, IOException
    {
        ...
        if (!isCover.isChecked() && ZipUtils.folderIsExists(path0 + "\\" + fileName))
        {
            Messagebox.show("导入的样式文件已存在！", "错误", Messagebox.OK,Messagebox.ERROR);
            return;
        }
