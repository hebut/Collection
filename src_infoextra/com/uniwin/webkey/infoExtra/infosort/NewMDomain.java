...
public class NewMDomain extends Window implements AfterCompose{

	
	Textbox name,code,issue;
	InfoDomainService infodomainService;
	WkTInfoSort infoSort;
	DomainListbox domainlist;
	...
	public void onSelect$domainlist() throws InterruptedException
	{
		Listitem item=domainlist.getSelectedItem();
		WkTInfoDomain d=(WkTInfoDomain) item.getValue();
//		if(d.getKiId().toString().trim().equals(p.getKptId(.toString().trim()))
//		{
//			List<WKTPersonTopic>  wpt=domainlist.findBykptId(p.getKptPid());
//			domainlist.initAllTopicSortSelect(wpt.get(0),p);
//		}
	}
