...
public class NewsauditDetailListRendererNew implements ListitemRenderer {
	...
	public NewsauditDetailListRendererNew(OriNewsService orinewsService,NewsServices newsService,
	...
	public void render(Listitem item, Object data) throws Exception {
		...
		if(getTask()==null){
		 ...
		 if(orinfo.getKoiTitle().trim().length()>26){
				String str1 = "";                                                      
				str1=orinfo.getKoiTitle().trim().substring(0,26);
		...
		else if(data instanceof WkTDistribute)
		{
			...
			if(getTask()==null){
			 ...
			 item.appendChild(c0);
			 Listcell c1=new Listcell();
			 if(info.getKiTitle().trim().length()>26){
					String str1 = "";                                                     
					str1=info.getKiTitle().trim().substring(0,26);
						c1=new Listcell(str1+"......");
				}else{
	...
	public void reloadList(){	 
		...
		// List writeList=newsService.getNewsOfChanelZG(etask.getKeId(),
		...
		readListbox.setItemRenderer(new NewsauditDetailListRenderer(
				orinewsService, newsService, readListModel, auditListModel,
				pubListModel, auditListbox, readListbox, pubListbox,applyList));
	...
	public void initAllWindow(List taskList) {
		...
		readListbox.setModel(readListModel);
		pubListbox.setModel(pubListModel);
		readListbox.setItemRenderer(new NewsauditDetailListRendererNew(
				orinewsService, newsService, readListModel, auditListModel,
				pubListModel, auditListbox, readListbox, pubListbox,applyList));
//		auditListbox.setModel(auditListModel);
//		auditListbox.setItemRenderer(new NewsauditDetailListRendererNew(
//				orinewsService, newsService, readListModel, auditListModel,
//				pubListModel, auditListbox, readListbox, pubListbox));
		
		pubListbox.setItemRenderer(new NewsauditDetailListRendererNew(orinewsService,newsService,readListModel,auditListModel,pubListModel,auditListbox,readListbox,pubListbox,applyList));		
		*/
		...
		pubListbox.setItemRenderer(new NewsauditDetailListRendererNew(
				...
				pubListModel, auditListbox, readListbox, pubListbox,applyList));
		readListbox.setModel(readListModel);
		readListbox.setItemRenderer(new NewsauditDetailListRendererNew(orinewsService,newsService,readListModel,auditListModel,pubListModel,auditListbox,readListbox,pubListbox,applyList));
