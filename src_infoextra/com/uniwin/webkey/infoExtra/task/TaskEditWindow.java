	...
	public void loadTaskList(){
		...
		if(tList.size()!=0){
			...
			public void render(final Listitem item, Object arg1) throws Exception {
				...
				WkTChanel c=taskService.findByFolderID(task.getKcId());
				Listcell c8=new Listcell(c.getKcName());
				...
				if(task.getKeStatus()==Long.parseLong("0")){
					...
					{
					c5.setLabel(task.TIME_TASK+"(每隔)");
					}
					else if(task.getKeDefinitetype().equals("1"))
					{
						c5.setLabel(task.TIME_TASK+"(每当)");
						}
					c5.setStyle("color:green");
				
					if(task.getKeDefinitetype().equals("0")){
						h=Integer.valueOf((task.getKeDefinite()/3600)+"");
						m=Integer.valueOf((task.getKeDefinite()/60-h*60)+"");
					}else{
				...
				Listcell c6=new Listcell(h.toString()+"时"+m.toString()+"分");
				Listcell c7=new Listcell(task.getKeTime());
				
				item.appendChild(c1);item.appendChild(c2);
				item.appendChild(c4);
				item.appendChild(c8);item.appendChild(c5);
				item.appendChild(c6);
				item.appendChild(c7);
				...
				menu5.setLabel("更改任务分类");
				menu5.setImage("/images/content/zy.jpg");
...
//						push.setDone();
				...
				menu5.addEventListener(Events.ON_CLICK, new EventListener(){
					public void onEvent(Event arg0) throws Exception {
						...
						TaskTree tt = (TaskTree)Executions.createComponents("/apps/infoExtra/content/task/tasktree.zul",null,null);
						tt.doHighlighted();
						tt.initWin(task);
						tt.addEventListener(Events.ON_CHANGE, new EventListener(){
							public void onEvent(Event arg0) throws Exception {
								initWindow(tasktype);
							}
	...
	public void EditTask(final WkTExtractask extractask){
		Configure configure=(Configure)Executions.createComponents("/apps/infoExtra/content/task/configure.zul",null,null);
	...
	public void deleteGuideAndReg(WkTExtractask extractTask){
		...
		List<WkTLinkurl> lList=linkService.findByKeId(extractTask.getKeId());
...
//		Callable myCallable = null;
		...
		if(eTask.getKePubtype()==Long.parseLong("0")){
			//动态添加
			Tabpanel tabpanel=(Tabpanel)Executions.createComponents("/apps/infoExtra/content/task/Tabpanel.zul", null, null);
			...
			Listbox listbox=(Listbox)Executions.createComponents("/apps/infoExtra/content/task/listbox.zul", null, null);
	...
	public void onClick$signSet(){
		...
		HtmlSign sign=(HtmlSign)Executions.createComponents("/apps/infoExtra/content/task/htmlSign.zul", null, null);
		sign.Init();
		sign.doHighlighted();
	}
