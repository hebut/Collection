...
public class TasktypeTreeComposer extends Window implements AfterCompose {
			...
			taskTree.setTreeitemRenderer(new TaskItemRenderer()
		     {
		    		public void render(final Treeitem item, Object data) throws Exception {
		    			...
		    			if(clist != null && clist.size()>= 0 && type!=null)
						{
							if(type.getKcId().toString().trim().equals(task.getKcId().toString().trim()))
						...
						cmenu1.addEventListener(Events.ON_CLICK, new EventListener(){
							public void onEvent(Event event) throws Exception {
								NewSortWindow w=(NewSortWindow)Executions.createComponents("/apps/infoExtra/content/tasktemp/newsort.zul", null,null);
								w.doHighlighted();
								w.initWindow(task);
						...
						cmenu2.addEventListener(Events.ON_CLICK, new EventListener(){
							public void onEvent(Event event) throws Exception {
								...
								else
								{
									...
									{
									ImportTypeWindow w=(ImportTypeWindow) Executions.createComponents("/apps/infoExtra/content/tasktemp/imports.zul", null,null);
								    w.doHighlighted();
								    w.initWindow(task);
									w.addEventListener(Events.ON_CHANGE, new EventListener(){
										public void onEvent(Event arg0) throws Exception {
						...
						menu4.addEventListener(Events.ON_CLICK, new EventListener(){
							public void onEvent(Event event) throws Exception {
								     TasktypeSortWindow w=(TasktypeSortWindow)Executions.createComponents("/apps/infoExtra/content/tasktemp/typeSort.zul", null, null);
									w.doHighlighted();
									w.initWindow(task);
									w.addEventListener(Events.ON_CHANGE, new EventListener(){
										public void onEvent(Event arg0) throws Exception {
						...
						menu2.addEventListener(Events.ON_CLICK, new EventListener(){
							public void onEvent(Event event) throws Exception {
								SortEditWindow s=(SortEditWindow)Executions.createComponents("/apps/infoExtra/content/tasktemp/sortedit.zul", null,null);
								s.doHighlighted();
								s.initWindow(task);
	...
	public void	windowsLoad(WkTChanel tasktype){
		if(tasktype!=null){
			taskCen.setTitle(org.zkoss.util.resource.Labels
	                .getLabel("chanel.ui.operationChanelName")+" : "+tasktype.getKcName());
			taskCen.getChildren().clear();
			editWindow= (TaskEditWindow)Executions.createComponents("/apps/infoExtra/content/tasktemp/tasklist.zul",taskCen, null);
